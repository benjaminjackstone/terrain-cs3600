...
gliReadTGA(FILE *fp, char *name)
{
  ...
  if (fseek(fp, 0L - sizeof(tgaFooter), SEEK_END)
    ...
    sprintf_s(error, "TGA: Cannot read footer from \"%s\"", name);
    if (verbose) printf("%s\n", error);
    return NULL;
  }  
  ...
  if (fseek(fp, 0, SEEK_SET) ||
    ...
    sprintf_s(error, "TGA: Cannot read header from \"%s\"", name);
    if (verbose) printf("%s\n", error);
    return NULL;
  }
  ...
  if (verbose && tgaHeader.idLength) {
    ...
    if (fread(idString, tgaHeader.idLength, 1, fp) != 1) {
      sprintf_s(error, "TGA: Cannot read ID field in \"%s\"", name);
      printf("%s\n", error);
    } else {
    ...
    if (tgaHeader.idLength && fseek(fp, tgaHeader.idLength, SEEK_CUR)) {
      sprintf_s(error, "TGA: Cannot skip ID field in \"%s\"", name);
      if (verbose) printf("%s\n", error);
      return NULL;
    }
  }
  ...
  if ((format == GL_BGRA_EXT && bpp != 32) ||
    ...
    sprintf_s(error, "TGA: channel sizes other than 8 bits are unimplemented");
    if (verbose) printf("%s\n", error);
    return NULL;
  }
  ...
  if (format == GL_COLOR_INDEX) {
    if (tgaHeader.colorMapType != 1) {
      sprintf_s(error, "TGA: indexed image has invalid color map type %d\n",
        tgaHeader.colorMapType);
      if (verbose) printf("%s\n", error);
      return NULL;
    }
  } else if (tgaHeader.colorMapType != 0) {
    sprintf_s(error, "TGA: non-indexed image has invalid color map type %d\n",
      tgaHeader.colorMapType);
    if (verbose) printf("%s\n", error);
    return NULL;
  }
  ...
  if (tgaHeader.colorMapType == 1) {
    ...
    if (length == 0) {
      sprintf_s(error, "TGA: invalid color map length %d", length);
      if (verbose) printf("%s\n", error);
      return NULL;
    }
    ...
    if (fread(cmap, pelbytes, length, fp) != (size_t) length) {
      sprintf_s(error, "TGA: error reading colormap (ftell == %ld)\n",
        ftell (fp));
      if (verbose) printf("%s\n", error);
      return NULL;
    }
